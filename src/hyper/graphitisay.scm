...
(define-public *graphitisay* '((vertex
                                ((uid . record))
                                ((label . string)
                                 (assoc . string))
                                ((label (label) (uid))))
...
(when (or (getenv "CHECK") (getenv "CHECK_GRAPHITISAY"))
  (format #t "* Testing graphitisay\n")
  (with-directory "/tmp/graphitisay"
                  (receive (cnx context) (apply wiredtiger-open* (cons "/tmp/graphitisay" *graphitisay*))
